...
select category, sum(stock) as  total_stocks from products
group by category having total_stocks>100 ;
...
SELECT ProductName, Price, Price + 1000 AS IncreasedPrice
FROM Products;

-- Subtraction
SELECT ProductName, Price, Price - 500 AS DiscountedPrice
FROM Products;
